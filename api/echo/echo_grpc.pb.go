...
const (
	...
	Echo_SayHelloBidiStream_FullMethodName = "/echo.Echo/SayHelloBidiStream"
)
...
type EchoClient interface {
	...
	SayHelloBidiStream(ctx context.Context, opts ...grpc.CallOption) (grpc.BidiStreamingClient[EchoRequest, EchoReply], error)
}
...
func (c *echoClient) SayHelloBidiStream(ctx context.Context, opts ...grpc.CallOption) (grpc.BidiStreamingClient[EchoRequest, EchoReply], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &Echo_ServiceDesc.Streams[0], Echo_SayHelloBidiStream_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
...
type Echo_SayHelloBidiStreamClient = grpc.BidiStreamingClient[EchoRequest, EchoReply]
...
type EchoServer interface {
	...
	SayHelloBidiStream(grpc.BidiStreamingServer[EchoRequest, EchoReply]) error
...
func (UnimplementedEchoServer) SayHelloBidiStream(grpc.BidiStreamingServer[EchoRequest, EchoReply]) error {
...
func _Echo_SayHelloBidiStream_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(EchoServer).SayHelloBidiStream(&grpc.GenericServerStream[EchoRequest, EchoReply]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type Echo_SayHelloBidiStreamServer = grpc.BidiStreamingServer[EchoRequest, EchoReply]
	...
	Streams: []grpc.StreamDesc{
		{
			...
			Handler:       _Echo_SayHelloBidiStream_Handler,
			ServerStreams: true,
			ClientStreams: true,
		},
	},
